...
class Builder:
    def __init__(self):
        ...
        if ostype == 'Linux':
            if 'aarch64' in machine or 'arm64' in machine:
                ...
                name = 'MIO-KITCHEN-linux.loongarch64.zip'
            else:
                name = 'MIO-KITCHEN-linux.x64.zip'
        elif ostype == 'Darwin':
            if 'x86_64' in machine or 'amd64' in machine:
                name = 'MIO-KITCHEN-macos.x64.zip'
            else:
                name = 'MIO-KITCHEN-macos.arm64.zip'
        else:
            # Windows
            import struct
            bits = struct.calcsize("P") * 8
            if bits == 64:
                name = 'MIO-KITCHEN-win7-11.x64.zip'
            else:
                name = 'MIO-KITCHEN-win7-11.x86.zip'
    ...
    def move_artifacts(self):
        ...
        # Новые имена артефактов
        artifacts = [
            'MIO-KITCHEN-win7-11.x86',
            'MIO-KITCHEN-win7-11.x64',
            'MIO-KITCHEN-linux.x64',
            'MIO-KITCHEN-macos.arm64',
            'MIO-KITCHEN-macos.x64'
        ]
        
        for artifact in artifacts:
            artifact_dir = artifact
            zip_file = f'{artifact}.zip'
            if os.path.exists(artifact_dir) and os.path.exists(f'{artifact_dir}/{zip_file}'):
                # Добавляем версию в имя: MIO-KITCHEN-win7-11.x86 -> MIO-KITCHEN-3.2.5-win7-11.x86
                parts = artifact.split('-', 1)  # ['MIO', 'KITCHEN-win7-11.x86']
                new_name = f'{parts[0]}-{ver}-{parts[1]}.zip'
                os.rename(f'{artifact_dir}/{zip_file}', new_name)
    ...
    def pyinstaller_build(self):
        ...
        if os.name == 'nt':
            ...
            PyInstaller.__main__.run(['tool.spec'])
    ...
    def config_folder(self):
        ...
        while_list = ['images', 'languages', 'licenses', 'module', 'temp', 'extra_flash', 'setting.ini', self.ostype,
                      ...
                      'Android', 'config', 'Darwin', 'Linux', 'Windows', 'keys',
                      'configs.json', 'context_rules.json', 'update-binary']
