        ...
        def call_func(*args, **kwargs):
            ...
            def wrapper(*a, **k):
                ...
                try:
                    ...
                    # Clean up task and stop animation when done
            ...
            task_real.start()
            # Don't join() - let it run asynchronously to avoid blocking GUI
            return return_value
...
class CustomControls:
    ...
    @staticmethod
    def combobox(master, textvariable: tk.Variable, values, text: str, state: str = 'normal'):
...
class ToolBox(ttk.Frame):
    ...
    class MagiskPatcher(Toplevel):
        ...
        def get_arch(self, apk_path=None):
...
def error(code, desc="unknown error"):
    try:
        if settings.debug_mode == 'No' and win.winfo_exists():
            win.withdraw()
...
class ModuleManager:
    ...
    def get_info(self, id_: str, item: str, default: str = None) -> Union[str, Dict[Any, Any], Any]:
        if not default:
            default = {}
...
class PackSuper(Toplevel):
    ...
    def read_list(self):
        ...
        if os.path.exists(parts_info):
            try:
                data = JsonEdit(parts_info).read().get('super_info')
                if data is None:
                    logging.debug('PackSupper:read_parts_info - super_info not found in parts_info, using defaults')
                else:
...
@animation
def pack_super(sparse: bool, group_name: str, size: int, super_type, part_list, del_=0, return_cmd=0,
               attrib='readonly',
               output_dir: str = None, work: str = None, block_device_name: str = 'None'):
    logging.info("=== pack_super START ===")
    logging.info(f"Parameters: sparse={sparse}, group_name={group_name}, size={size}, super_type={super_type}")
    logging.info(f"part_list={part_list}, del_={del_}, attrib={attrib}, block_device_name={block_device_name}")
    
    if not block_device_name:
        block_device_name = 'super'
    if not work:
        work = project_manger.current_work_output_path()  # Images are in Output/, not Source/
    if not output_dir:
        output_dir = project_manger.current_work_output_path()
    
    logging.info(f"work path: {work}")
    logging.info(f"output_dir path: {output_dir}")
    ...
    for part in part_list:
        ...
        if part not in lb_c:
            lb_c.append(part)
    part_list = lb_c
    logging.info(f"Cleaned part_list: {part_list}")
    
    # Check which images exist
    for part in part_list:
        img_path = f'{work}{part}.img'
        img_a_path = f'{work}{part}_a.img'
        logging.info(f"Checking {part}: {img_path} exists={os.path.exists(img_path)}, {img_a_path} exists={os.path.exists(img_a_path)}")
        if not os.path.exists(img_path) and os.path.exists(img_a_path):
            logging.info(f"Renaming {img_a_path} to {img_path}")
            try:
                os.rename(img_a_path, img_path)
            except Exception as e:
                logging.exception(f'Failed to rename {img_a_path}: {e}')
    ...
    if super_type == 1:
        ...
        for part in part_list:
            ...
            if os.path.exists(img_path):
                ...
                logging.info(f"Adding partition {part}: size={img_size}, path={img_path}")
                command += ['--partition', f"{part}:{attrib}:{img_size}:{group_name}",
        ...
        for part in part_list:
            ...
            if os.path.exists(img_path):
                img_size = os.path.getsize(img_path)
                logging.info(f"Adding partition {part}_a: size={img_size}, path={img_path}")
                command += ['--partition',
                            f"{part}_a:{attrib}:{img_size}:{group_name}_a",
    ...
    logging.info("Calling lpmake...")
    ret = call(command)
    logging.info(f"lpmake returned: {ret}")
    
    if ret == 0:
        if os.access(output_dir + "super.img", os.F_OK):
            logging.info(f"super.img created successfully at {output_dir}super.img")
                ...
                for img in part_list:
                    if os.path.exists(f"{work}{img}.img"):
                        try:
                            ...
                            logging.info(f"Deleted source image: {work}{img}.img")
                        except Exception as e:
                            logging.exception(f'Failed to delete {work}{img}.img: {e}')
        else:
            logging.error(f"lpmake succeeded but super.img not found at {output_dir}super.img")
            win.message_pop(lang.warn10)
            return 1
    else:
        logging.error(f"lpmake failed with return code {ret}")
        win.message_pop(lang.warn10)
        return 1
    
    logging.info("=== pack_super END ===")
    return 0
...
class StdoutRedirector:
    def __init__(self, text_widget: Text, error_=False):
        ...
        self.error_info = ''
        self.w = 0
        self.flush = self._safe_flush
        create_thread(self.loop)
        create_thread(self.loop2)
    
    def _safe_flush(self):
        """Safe flush that won't crash on application exit"""
        if self.error_info:
            try:
                # Check if the main window still exists before calling error()
                if hasattr(win, 'winfo_exists') and win.winfo_exists():
                    error(1, self.error_info)
            except (TclError, Exception):
                # Application is shutting down, just log to console
                try:
                    print(f"Error during shutdown: {self.error_info}", file=sys.__stderr__)
                except Exception:
                    pass
    ...
    def write(self, string):
        ...
        if self.error:
            ...
            try:
                ...
                # Fallback: replace problematic characters for Windows 7 compatibility
                try:
                    # Try to encode/decode with error handling
                    safe_string = string.encode('utf-8', errors='replace').decode('utf-8', errors='replace')
                    logging.error(safe_string)
                except Exception:
                    # Last resort: convert to ASCII
                    try:
                        safe_string = string.encode('ascii', errors='replace').decode('ascii')
                        logging.error(safe_string)
                    except Exception:
                        # If all else fails, log a generic error message
                        logging.error('[Error: Unable to encode log message]')
            except Exception as e:
                # Catch any other logging errors to prevent crashes
                try:
    ...
    def loop(self):
        while True:
            try:
                if self.w and self.text_space.winfo_exists():
...
def unpackrom(ifile: str) -> None:
    ...
    if ftype != 'unknown':
        ...
        if os.path.exists(os.path.join(project_dir, file_name)):
            ...
            # Don't rename payload.bin to payload.img - it needs to stay as .bin
            elif file_name.endswith(".bin") and not file_name.startswith("payload"):
...
@animation
def unpack(chose: Union[List, Dict], form: str = '') -> bool:
    # Try to enable case sensitivity on Windows 10+ (not available on Windows 7)
    if os.name == 'nt':
        if windll.shell32.IsUserAnAdmin():
            try:
                ensure_dir_case_sensitive(project_manger.current_work_path())
            except (OSError, Exception) as e:
                # Silently ignore on Windows 7 or if WSL is not enabled or if directory is not empty
                # OSError [WinError 87] means the API is not available (Windows 7)
                # OSError [WinError 145] means the directory is not empty
                if hasattr(e, 'winerror') and e.winerror in [87, 145]:
                    logging.debug(f'Case sensitivity not available: {e}')
                else:
                    logging.exception('Error setting case sensitivity')
    if not project_manger.exist():
        win.message_pop(lang.warn1)
        return False
...
@animation
def datbr(work: str, name: str, brl: Union[str, int], dat_ver: int = 4):
    """
...
def __init__tk(args):
    if not os.path.exists(temp):
        re_folder(temp, quiet=True)
    # Configure logging with UTF-8 encoding (compatible with Python 3.8+ and Windows 7+)
    # Create log file with UTF-8 encoding first
    if not os.path.exists(tool_log):
        try:
            with open(tool_log, 'w', encoding="utf-8", newline="\n") as f:
                pass  # Just create empty file
        except (OSError, IOError):
            # Fallback for systems with strict file permissions
    ...
    if not states.development:
        ...
        # Approach 1: Try with encoding parameter (Python 3.9+)
        try:
            file_handler = logging.FileHandler(tool_log, mode='w', encoding='utf-8')
        except TypeError:
            # Approach 2: Try without encoding parameter (Python 3.8 and older)
            try:
                file_handler = logging.FileHandler(tool_log, mode='w')
                # Try to set encoding on the stream if possible
                ...
                # Approach 3: Last resort - create handler without any special settings
                file_handler = logging.FileHandler(tool_log)
        
        if file_handler:
            file_handler.setLevel(logging.INFO)  # Changed from DEBUG to INFO to reduce log size
            file_handler.setFormatter(logging.Formatter(log_format))
            
            # Configure root logger
            logging.root.setLevel(logging.INFO)  # Changed from DEBUG to INFO
            logging.root.addHandler(file_handler)
            
            # Suppress verbose logging from third-party libraries
            logging.getLogger('PIL').setLevel(logging.WARNING)  # Suppress PIL/Pillow DEBUG logs
            logging.getLogger('PIL.PngImagePlugin').setLevel(logging.WARNING)
            logging.getLogger('PIL.Image').setLevel(logging.WARNING)
            logging.getLogger('urllib3').setLevel(logging.WARNING)
            logging.getLogger('requests').setLevel(logging.WARNING)
    else:
        # For development mode, use console with UTF-8 encoding
        console_handler = logging.StreamHandler(sys.stdout)
        console_handler.setLevel(logging.DEBUG)
        console_handler.setFormatter(logging.Formatter(log_format))
        
        # Try to set UTF-8 encoding on the stream
        try:
            console_handler.stream = open(sys.stdout.fileno(), mode='w', encoding='utf-8', buffering=1)
        except (AttributeError, OSError, TypeError, ValueError):
            # Fallback to default stream if reconfiguration fails
            # This can happen on Windows 7 or in compiled executables
            pass
